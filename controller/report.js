...
//             answered_calls: Number(rows[0].answered_calls) || 0,
//             missed_calls: Number(rows[0].missed_calls) || 0
//         };

//         res.json(data);
//     } catch (err) {
//         console.error(err);
//         res.status(500).json({ message: "Error fetching inbound call report" });
//     }
// };
...
//         const row = rows[0] || {};
//         let formattedDuration = "0h 0m 0s";
//         if (row.call_duration) {
//             const [hours, minutes, seconds] = row.call_duration.split(':').map(Number);
...
//         res.json(data);
//     } catch (err) {
//         console.error("Error fetching inbound call report:", err);
//         res.status(500).json({ message: "Error fetching inbound call report" });
//     }
// };
...
//         const [inboundRows] = await pool.query(inboundQuery, params);
//         const [outboundRows] = await pool.query(outboundQuery, params);

//         const formatDuration = (d) => {
//             if (!d) return "0h 0m 0s";
//             const [h, m, s] = d.split(':').map(Number);
//             return `${h}h ${m}m ${s}s`;
//         };

//         const inbound = {
//             total_calls: Number(inboundRows?.total_calls) || 0,
//             answered_calls: Number(inboundRows?.answered_calls) || 0,
//             missed_calls: Number(inboundRows?.missed_calls) || 0,
//             call_duration: formatDuration(inboundRows?.call_duration)
//         };

//         const outbound = {
//             total_calls: Number(outboundRows?.total_calls) || 0,
//             answered_calls: Number(outboundRows?.answered_calls) || 0,
//             busy_calls: Number(outboundRows?.busy_calls) || 0,
//             noanswer_calls: Number(outboundRows?.noanswer_calls) || 0,
//             notreachable_calls: Number(outboundRows?.notreachable_calls) || 0,
//             call_duration: formatDuration(outboundRows?.call_duration)
//         };
...
//         res.status(500).json({ message: "Error fetching combined report" });
//     }
// };
...
// ***********************************     All    Inbound Outbound And Call Insight **********
